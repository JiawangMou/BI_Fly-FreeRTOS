...
void float_quat_differential(struct FloatQuat *q_out, struct FloatRates *w, float dt)
{
  ...
  const float c2 = cos(dt * v_norm / 2.0f);
  const float s2 = sin(dt * v_norm / 2.0f);
  if (v_norm < 1e-8f) {
    q_out->qi = 1;
    q_out->qx = 0;
    q_out->qy = 0;
...
void float_quat_integrate_fi(struct FloatQuat *q, struct FloatRates *omega, float dt)
{
  ...
  const float qy = q->qy;
  const float qz = q->qz;
  const float dp = 0.5f * dt * omega->p;
  const float dq = 0.5f * dt * omega->q;
  const float dr = 0.5f * dt * omega->r;
...
{
  const float qi2_M1_2  = q->qi * q->qi - 0.5f;
...
void float_quat_of_orientation_vect(struct FloatQuat *q, const struct FloatVect3 *ov)
{
  ...
  if (ov_norm < 1e-8f) {
    ...
    const float s2_normalized = sinf(ov_norm / 2.0f) / ov_norm;
    q->qi = cosf(ov_norm / 2.0f);
...
void float_quat_of_rmat(struct FloatQuat *q, struct FloatRMat *rm)
{
  ...
  if (tr > 0) {
    const float two_qi = sqrtf(1.f + tr);
    const float four_qi = 2.f * two_qi;
    q->qi = 0.5f * two_qi;
    ...
    if (RMAT_ELMT(*rm, 0, 0) > RMAT_ELMT(*rm, 1, 1) &&
      ...
      const float two_qx = sqrtf(RMAT_ELMT(*rm, 0, 0) - RMAT_ELMT(*rm, 1, 1)
                                 - RMAT_ELMT(*rm, 2, 2) + 1);
      const float four_qx = 2.f * two_qx;
      ...
      const float four_qy = 2.f * two_qy;
      ...
      const float four_qz = 2.f * two_qz;
...
bool float_mat_inv_2d(float inv_out[4], float mat_in[4])
{
  ...
  if (fabsf(det) < 1e-4f) { return 1; } //not invertible
