...
bool spiTest(void) { return isInit; }
...
bool spiExchange(size_t length, const uint8_t* data_tx, uint8_t* data_rx)
{
    ...
    // 清除中断标志位
    DMA_ClearFlag(
        SPI_TX_DMA_STREAM, DMA_FLAG_FEIF4 | DMA_FLAG_DMEIF4 | DMA_FLAG_TEIF4 | DMA_FLAG_HTIF4 | DMA_FLAG_TCIF4);
    DMA_ClearFlag(
        SPI_RX_DMA_STREAM, DMA_FLAG_FEIF3 | DMA_FLAG_DMEIF3 | DMA_FLAG_TEIF3 | DMA_FLAG_HTIF3 | DMA_FLAG_TCIF3);
...
void spiBeginTransaction(void) { xSemaphoreTake(spiMutex, portMAX_DELAY); }

void spiEndTransaction() { xSemaphoreGive(spiMutex); }
