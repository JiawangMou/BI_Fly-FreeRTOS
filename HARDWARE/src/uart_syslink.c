...
void uartslkSendData(u32 size, u8* data)
{
    ...
    for (i = 0; i < size; i++) {
        ...
        // USART_SendData(UARTSLK_TYPE,data[i]);
    }
}
/*中断方式发送原始数据*/
...
void uartslkIsr(void)
{
    ...
    {
        // data[i] = (u8)USART_ReceiveData(UARTSLK_TYPE);
        // 		i++;
        u8 rxDataInterrupt = (u8)USART_ReceiveData(UARTSLK_TYPE);
        xQueueSendFromISR(uartslkDataDelivery, &rxDataInterrupt, &xHigherPriorityTaskWoken);
        USART_ClearITPendingBit(UARTSLK_TYPE, USART_IT_RXNE);
