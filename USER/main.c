...
void startTask(void *arg)
{
    ...
    xTaskCreate(stabilizerTask, "STABILIZER", 450, NULL, 4, NULL);    /*创建姿态任务*/
	// xTaskCreate(opticalFlowTask, "OPTICAL_FLOW", 300, NULL, 4, NULL); /*创建光流模块任务*/
    // xTaskCreate(hallTask, "HALL", 300, NULL, 4, NULL); /*创建Hall模块任务*/
    //    xTaskCreate(expModuleMgtTask, "EXP_MODULE", 150, NULL, 1, NULL);	/*创建扩展模块管理任务*/
    //以下为测试代码
    xTaskCreate(ledTask, "LEDTASK", 150, NULL, 2, NULL);
